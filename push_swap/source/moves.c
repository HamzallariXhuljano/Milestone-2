...
/*   Updated: 2025/02/12 23:11:52 by xhamzall         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	while (d->pos_a > d->size_a / 2 && d->index > d->size_b / 2)
	{
		rrr(a, b);
		d->pos_a--;  // ✅ Decrement instead of increment
		d->index--;  // ✅ Decrement instead of increment
	}
}
...
void	movement(t_swap **a, t_swap **b, t_data *d)
{
	...
	// Eseguire rr o rrr solo se entrambi si trovano nella stessa metà
	...
	// Ora sposta `a` nella posizione giusta
	movement_a(a, d->pos_a);

	// Infine, esegui `pa()` per inserire l'elemento correttamente
	pa(a, b);
}
